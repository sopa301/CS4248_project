...
def build_model(config, tokenizer, model_path):
    ...
    #TODO: need to implement the model
    model = AutoModelForSequenceClassification.from_pretrained(model_path, num_labels=2, output_hidden_states=True, ignore_mismatched_sizes=True)
...
def get_dataloader(config, tokenizer):
    ...
    train_dataset = EmoteDataset(
        filename_ls_path=train_config['filename_ls_path'],
    ...
    val_dataset = EmoteDataset(
        filename_ls_path=val_config['filename_ls_path'],
...
def main_worker(rank: int, config: dict):
    ...
    try:
        ...
        model = build_model(config, tokenizer, model_path)
