...
class MultimodalDataset(Dataset):
    def __init__(self, encodings, labels, strategies, sensitivities, img_dir, img_transform=None, debug=False):
    ...
    def __getitem__(self, idx):
        ...
        item['sensitivities'] = self.sensitivities[original_idx].clone().detach()
        ...
        if self.debug:
            ...
            if 'input_ids' in item and self.tokenizer is not None:
                try:
                    ...
                    print(f"Index: {original_idx}, Image: {img_path}, Label: {item['labels'].item()}, Strategy: {item['strategies'].item()}, positional_sensitivity: {item['sensitivities'].item()}")
                    print(f"Text: {text}")
                    print("-" * 50)
                except Exception as e:
                    print(f"Index: {original_idx}, Image: {img_path}, Label: {item['labels'].item()}, Strategy: {item['strategies'].item()}, positional_sensitivity: {item['sensitivities'].item()}")
