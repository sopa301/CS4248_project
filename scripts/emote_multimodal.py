...
if __name__ == '__main__':
    ...
    # Create the datasets
    if config.use_images:
        train_dataset = MultimodalDataset(train_inputs, train_labels, train_strategies, config.train_img_dir, train_data)
        val_dataset = MultimodalDataset(val_inputs, val_labels, val_strategies, config.val_img_dir, val_data)
        test_dataset = MultimodalDataset(test_inputs, test_labels, test_strategies, config.test_img_dir, test_data)
        all_dataset = MultimodalDataset(all_inputs, all_labels, all_strategies, config.train_img_dir, all_data)  # Using train_img_dir as a fallback
    else:
    ...
    all_loader = DataLoader(all_dataset, batch_size=batch_size, collate_fn=custom_collate_fn)
    ...
    if config.use_images:
        from seq_img_with_pos_model import EmoteMultimodalModel
