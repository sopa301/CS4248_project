...
class MultiImageEncoder(nn.Module):
    def __init__(self, swin_model, output_dim=768, max_num_images=20):
        super().__init__()
        self.swin = swin_model
        self.image_proj = nn.Linear(swin_model.norm.normalized_shape[0], output_dim)
        self.positional_embedding = nn.Parameter(torch.randn(max_num_images, output_dim))
    ...
    def forward(self, batch_of_image_lists):
        ...
        for image_list in batch_of_image_lists:
            ...
            if seq_len > max_seq_len:
                raise ValueError(f"Too many images ({seq_len} > {max_seq_len})")

            pos_embed = self.positional_embedding[:seq_len]
            fused_input = projected + pos_embed
            fused_input = fused_input.unsqueeze(1)  # [seq_len, 1, dim]
...
class EmoteMultimodalModel(nn.Module):
    def __init__(self, config, num_labels=2):
        ...
        self.text_dim = 768
        if config.model_name == 'bart-large' or config.model_name == 'roberta-large':
            self.text_dim = 1024
