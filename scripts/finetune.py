...
class EmoteTrainer:
    ...
    def _run_epoch(self, epoch, train, loader=None):
        ...
        for batch_idx, batch in enumerate(batch_pbar):
            ...
            batch = {k: v.to(self.device) if k != 'images' else v for k, v in batch.items()}
        ...
        # Add this inside the `_run_epoch` method, after the strategy accuracy calculations

        # # Calculate accuracy for each pos_sensitivity
        # pos_sensitivity_correct = [0] * 5  # Assuming pos_sensitivity values range from 0 to 4
        # pos_sensitivity_total = [0] * 5

        # for i in range(len(preds)):
        #     pos_sensitivity_value = int(pos_sensitivity[i].item())  # Get the pos_sensitivity value
        #     pos_sensitivity_total[pos_sensitivity_value] += 1
        #     if preds[i] == current_true_labels[i]:
        #         pos_sensitivity_correct[pos_sensitivity_value] += 1

        # # Print and log the accuracy for each pos_sensitivity
        # for i in range(len(pos_sensitivity_correct)):
        #     if pos_sensitivity_total[i] != 0:
        #         accuracy = round(pos_sensitivity_correct[i] / pos_sensitivity_total[i] * 100, 1)
        #         print(f'Positional Sensitivity {i} accuracy: {accuracy}, {pos_sensitivity_correct[i]} / {pos_sensitivity_total[i]}')
        #         if str_ in ['Valid', 'Test']:
        #             with open(self.fp_txt, 'a') as f:
        #                 f.write(f'Positional Sensitivity {i}, {accuracy}, {pos_sensitivity_correct[i]} / {pos_sensitivity_total[i]}\n')
        #     else:
        #         print(f'No instances of Positional Sensitivity {i} found')
