...
class EmoteTrainer:
    ...
    def train(self, epochs):
        ...
        for epoch in epoch_pbar:
            ...
            val_acc, val_acc_strategy, val_acc_sensitivity = self._run_epoch(epoch=epoch, train=False, loader=self.val_loader)
            
            # Run test with progress bar
            test_acc, test_acc_strategy, test_acc_sensitivity = self._run_epoch(epoch=epoch, train=False, loader=self.test_loader)
            ...
            if val_acc > self.best_val_acc:
                ...
                small_dict['Overall'] = test_acc
                small_dict['Sensitivity 1'] = test_acc_sensitivity[0]
                small_dict['Sensitivity 2'] = test_acc_sensitivity[1]
    ...
    def _run_epoch(self, epoch, train, loader=None):
        ...
        for batch_idx, batch in enumerate(batch_pbar):
            # Separate strategies and sensitivities from the rest of the batch data
            # strategies = batch.pop('strategies').to(self.device)
            sensitivities = batch.pop('sensitivities').to(self.device)
            ...
            sensitivity_types.extend(sensitivities.cpu().numpy().tolist())

            current_true_labels = batch['labels'].cpu().numpy().tolist()
            # current_strategies = strategies.cpu().numpy().tolist()
            current_sensitivities = sensitivities.cpu().numpy().tolist()

            for i in range(len(preds)):
                if current_true_labels[i] == 1:  # Only count for positive labels for now
                    # strategies_total[current_strategies[i]] += 1
                    sensitivities_total[current_sensitivities[i]-1] += 1
        ...
        # Calculate accuracy for each strategy, sensitivity
        strategy_acc = []
        sensitivity_acc = []
        ...
        return overall_accuracy, strategy_acc, sensitivity_acc
